...
# auto_scale_lr = dict(base_batch_size=512, enable=True) ## enables.
# Will change LR based on actual batch size this base batch size
...
codec = dict(
    ...
    heatmap_size=(int(image_size[0]/scale), int(image_size[1]/scale)), sigma=sigma,
) ## sigma is 2 for 256
...
model = dict(
    ...
    head=dict(
        ...
        ## loss only for top 64 keypoints. for finetuning later.
        # loss=dict(type='KeypointOHKMMSELoss', use_target_weight=True, topk=128),
        decoder=codec),
    test_cfg=dict(
        flip_test=True,
        flip_mode="heatmap",
